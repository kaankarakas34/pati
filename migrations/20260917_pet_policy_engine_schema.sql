...
-- Automatically backfill size_categories based on weight_limit
UPDATE public.hotels
SET size_categories = CASE
  WHEN weight_limit = 0 OR weight_limit IS NULL THEN '["small", "medium", "large"]'::jsonb
  WHEN weight_limit >= 20 THEN '["small", "medium", "large"]'::jsonb
  WHEN weight_limit >= 10 THEN '["small", "medium"]'::jsonb
  ELSE '["small"]'::jsonb
END
WHERE size_categories IS NULL OR size_categories = '[]'::jsonb;

-- Automatically backfill pet_fee_type based on extra_fee
UPDATE public.hotels
SET pet_fee_type = CASE
  WHEN extra_fee = 'no' THEN 'free'
  WHEN extra_fee = 'yes' THEN 'per_night'
  ELSE 'unknown'
END
WHERE pet_fee_type = 'unknown';
