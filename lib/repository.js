...
import { filterStaticGuides, getFlagshipGuideBySlug } from './flagship-guides.js';

let cachedExperiences = null;
export function getStaticExperiences() {
  if (cachedExperiences) return cachedExperiences;
  try {
    const raw = readFileSync(new URL('../data/experiences.json', import.meta.url), 'utf8');
    cachedExperiences = JSON.parse(raw);
  } catch {
    cachedExperiences = [];
  }
...
export function filterStaticExperiences(query = {}) {
  let list = getStaticExperiences();
  if (query.id) {
  ...
  if (query.pet && query.pet !== 'all') {
    list = list.filter(item => Array.isArray(item.allowedPets) && item.allowedPets.includes(query.pet));
  }
  if (query.q) {
    const term = slugify(String(query.q));
    if (term.length >= 2) {
      list = list.filter(item =>
        slugify(item.city || '').includes(term) ||
        slugify(item.district || '').includes(term) ||
        slugify(item.name || '').includes(term) ||
        slugify(item.description || '').includes(term)
      );
    }
  }
  return list;
...
let cachedBoardings = null;
export function getStaticBoardings() {
  if (cachedBoardings) return cachedBoardings;
  try {
    const raw = readFileSync(new URL('../data/parsed-pet-hotels.json', import.meta.url), 'utf8');
    const items = JSON.parse(raw);
    cachedBoardings = items.map(mapRow);
  } catch {
    cachedBoardings = [];
  }
  return cachedBoardings;
}

export function filterStaticBoardings(query = {}) {
  let list = getStaticBoardings();
  if (query.id) {
    list = list.filter(item => item.id === query.id);
  }
  if (query.citySlug) {
    list = list.filter(item => slugify(item.city || '') === query.citySlug);
  }
  if (query.districtSlug) {
    list = list.filter(item => slugify(item.district || '') === query.districtSlug);
  }
  if (query.nameSlug) {
    list = list.filter(item => slugify(item.name || '') === query.nameSlug);
  }
  if (query.city && query.city !== 'all') {
    const target = slugify(query.city);
    list = list.filter(item => slugify(item.city || '') === target);
  }
  if (query.district && query.district !== 'all') {
    const target = slugify(query.district);
    list = list.filter(item => slugify(item.district || '') === target);
  }
  if (query.category && query.category !== 'all') {
    list = list.filter(item => item.category === query.category || item.serviceType === query.category);
  }
  if (query.pet && query.pet !== 'all') {
    list = list.filter(item => Array.isArray(item.allowedPets) && item.allowedPets.includes(query.pet));
  }
  if (query.feature) {
    const feats = Array.isArray(query.feature) ? query.feature : [query.feature];
    for (const f of feats) {
      if (!f || f === 'all') continue;
      const lower = String(f).toLowerCase();
      list = list.filter(item => Array.isArray(item.features) && item.features.some(feat => String(feat).toLowerCase().includes(lower)));
    }
  }
  if (query.q) {
    const term = slugify(String(query.q));
    if (term.length >= 2) {
      list = list.filter(item =>
        ...
        slugify(item.district || '').includes(term) ||
        slugify(item.name || '').includes(term) ||
        slugify(item.address || '').includes(term) ||
        slugify(item.description || '').includes(term)
      );
    }
  }
...
const summaryFields = new Set('id slug name city district type category image_url gallery_images allowed_pets features quiz_tags suitability weight_limit extra_fee base_trust_score verified price address phone website map_url pet_policy camera_support title summary short_answer author published_at updated_at starts_at ends_at status placement target_url sponsor last_verified'.split(' '));
...
export function createRepository(pool) {
  ...
  async function page(table,query={},detail=false) {
    ...
    try {
      ...
      dbRows = res.rows;
      dbSuccess = true;
      if (table !== 'experiences' && table !== 'boardings' && table !== 'guides') {
        return pageResult(dbRows.map(mapRow),built.limit);
      }
    } catch (err) {
      if (table !== 'experiences' && table !== 'boardings' && table !== 'guides') throw err;
    }
    if (table === 'experiences') {
      const dbItems = dbRows.map(mapRow);
      const dbIds = new Set(dbItems.map(item => item.id));
      const staticItems = filterStaticExperiences(query);
      ...
      return pageResult(merged, built.limit);
    }
    if (table === 'boardings') {
      const dbItems = dbRows.map(mapRow);
      const dbIds = new Set(dbItems.map(item => item.id));
      const staticItems = filterStaticBoardings(query);
      const merged = [...dbItems];
      for (const item of staticItems) {
        if (!dbIds.has(item.id)) {
          merged.push(item);
    ...
    if (table === 'guides') {
      const dbItems = dbRows.map(mapRow);
      const dbIds = new Set(dbItems.map(item => item.id));
      const staticItems = filterStaticGuides(query);
      const merged = [...dbItems];
      for (const item of staticItems) {
        if (!dbIds.has(item.id)) {
          merged.push(item);
    ...
    return pageResult([], built.limit);
  }
  async function one(table,id) {
    const byId = (await page(table,{id:recordId(id),limit:1},true)).data[0];
    if (byId) return byId;
    if (table === 'guides') {
      const bySlug = (await page(table,{slug:recordId(id),limit:1},true)).data[0];
      if (bySlug) return bySlug;
      return getFlagshipGuideBySlug(id) || null;
    }
    if (table === 'boardings') {
      return getStaticBoardings().find(b => b.id === id) || null;
    }
    return null;
  }
