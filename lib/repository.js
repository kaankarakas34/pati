...
export function createRepository(pool) {
  ...
  async function page(table,query={},detail=false) {
    ...
    try {
      ...
      if (res.rows.length > 0 || table !== 'experiences') {
        return pageResult(res.rows.map(mapRow),built.limit);
      }
    } catch (err) {
      if (table !== 'experiences') throw err;
    }
    if (table === 'experiences') {
      const fallbackItems = filterStaticExperiences(query);
      return pageResult(fallbackItems, built.limit);
    }
