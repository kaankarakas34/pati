...
export function renderBoardingPreRenderHtml(boarding, relatedBoardings = []) {
  ...
  const regionalTags = [
    ...
    `${boarding.district} kedi oteli`,
    `${boarding.city} köpek pansiyonu`,
    `${boarding.city} ${boarding.category?.toLowerCase() || 'pet bakımı'}`,
    `${boarding.district} pet pansiyon`,
    `en iyi ${boarding.city} pet otelleri`,
    `güvenilir pet bakım merkezi ${boarding.city}`
  ];
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <article class="space-y-6">
        <header class="space-y-2">
          ...
          <p class="text-sm text-gray-600">${escapeHtml(boarding.district || '')}, ${escapeHtml(boarding.city || '')} — Son Teyit: Eylül 2026</p>
        </header>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class="md:col-span-2 space-y-6">
            ...
            <!-- GEO/VEO Direct Answer Box -->
            <div class="bg-brand-navy-light/40 border-2 border-brand-navy/15 rounded-3xl p-5 md:p-6 text-sm">
              <h2 class="font-title font-bold text-brand-navy text-base mb-2 flex items-center gap-2">
                <span>💡</span> ${escapeHtml(boarding.name)} Evcil Hayvan Konaklama ve Bakım Koşulları
              </h2>
              <p class="text-gray-800 leading-relaxed font-medium text-xs md:text-sm">
                <strong>${escapeHtml(boarding.name)}</strong>, ${escapeHtml(boarding.city)} ili ${escapeHtml(boarding.district)} ilçesinde kedi ve köpek kabul eden doğrulanmış bir evcil hayvan oteli ve bakım merkezidir. 
                Gözetim altında hijyenik odalar, serbest oyun saatleri ve uzman personel hizmeti sağlanmaktadır.
                Girişte güncel aşı karnesi ibrazı zorunludur.
              </p>
            </div>
            ...
            <section class="p-4 bg-gray-50 border border-gray-200 rounded-2xl">
              <h3 class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Bölgesel Hizmet ve Arama Terimleri</h3>
              <div class="flex flex-wrap gap-1.5">
                ${regionalTags.map(tag => `
                  <span class="text-3xs bg-white border border-gray-300 text-gray-700 px-2 py-1 rounded-md font-medium">
                    #${escapeHtml(tag)}
                  </span>
                `).join('')}
              </div>
