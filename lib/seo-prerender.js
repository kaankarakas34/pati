...
import { DOG_WALKING_GUIDES, CITY_GUIDE_INTROS, VENUE_TYPES } from '../src/data/dogWalkingGuides.js';
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class="md:col-span-2 space-y-6">
            ...
            <section class="border border-brand-beige rounded-2xl p-5 bg-brand-cream/30 space-y-3">
              <h2 class="text-base font-bold font-title text-brand-navy">Doğrulanmış Evcil Hayvan Kabul Şartları</h2>
              <ul class="grid grid-cols-1 sm:grid-cols-2 gap-2.5 text-xs text-gray-700">
                <li><strong>Kabul Edilen Türler:</strong> ${(hotel.allowedPets || ['dog', 'cat']).join(', ').replace('dog', 'Köpek').replace('cat', 'Kedi')}</li>
                <li><strong>Kilo Sınırı:</strong> ${hotel.weightLimit > 0 ? `${hotel.weightLimit} kg sınır` : 'Kilo sınırı aranmıyor (Büyük ırk uygun)'}</li>
                <li><strong>Ek Ücret:</strong> ${hotel.extraFee === 'no' ? 'Ek ücret alınmaz (Ücretsiz)' : (hotel.extraFee || 'Tesisle teyit edilmelidir')}</li>
                <li><strong>Odada Yalnız Kalabilme:</strong> ${hotel.canLeaveInRoomAlone ? 'Evet, odada yalnız kalabilir' : 'Hayır, odada gözetimsiz bırakılamaz'}</li>
                <li><strong>Giriş Şartları:</strong> Aşı karnesi ve mikroçip zorunludur</li>
                <li><strong>Doğrulama Durumu:</strong> ${escapeHtml(hotel.infoSource || 'Patili.co Editör Ekibi Teyitli')} (${escapeHtml(hotel.lastVerified || 'Eylül 2026')})</li>
              </ul>
            </section>
...
export function renderGuidePreRenderHtml(guide) {
  ...
  const authorRole = typeof guide.author === 'object' && guide.author?.role
    ? guide.author.role
    : (guide.clusterTitle || 'Uzman Araştırma ve İçerik Kurulu');

  const clusterBreadcrumb = guide.clusterTitle
    ? `<li class="text-gray-400">/</li><li class="text-gray-600">${escapeHtml(guide.clusterTitle)}</li>`
    : '';

  const clusterBadge = guide.clusterTitle
    ? `<span class="bg-brand-navy text-white px-3 py-1 rounded-full font-bold uppercase text-3xs tracking-wider">${escapeHtml(guide.clusterTitle)}</span>`
    : '';

  const categoryLinkHtml = guide.categoryLink
    ? `<div class="my-6 p-4 bg-brand-cream/60 border border-brand-beige rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3">
        <div class="text-xs text-gray-700">
          <span class="font-bold text-brand-navy block">🐾 İlgili Kategori Listesi:</span>
          <span>Bu rehberde bahsi geçen hizmet ve işletmeleri inceleyin.</span>
        </div>
        <a href="${escapeHtml(guide.categoryLink)}" class="px-4 py-2 bg-brand-navy text-white text-xs font-bold rounded-xl hover:bg-brand-navy-hover transition-colors whitespace-nowrap">
  ...
  if (Array.isArray(guide.faq) && guide.faq.length > 0) {
    const items = guide.faq.map(qna => {
      const q = escapeHtml(qna.question || qna.q || '');
      const a = escapeHtml(qna.answer || qna.a || '');
      return `<div class="bg-white border border-brand-navy/10 p-5 rounded-2xl">
        <h4 class="font-bold text-gray-900 text-sm md:text-base flex items-start gap-2">
          <span class="text-brand-orange">S:</span>
          <span>${q}</span>
        </h4>
        <p class="text-xs md:text-sm text-gray-600 mt-2 pl-5 leading-relaxed">${a}</p>
      </div>`;
    }).join('\n');
    faqHtml = `<section class="space-y-4 pt-8 border-t border-brand-beige mt-8">
  ...
  if (Array.isArray(guide.internalLinks) && guide.internalLinks.length > 0) {
    const links = guide.internalLinks.map(l => {
      const url = escapeHtml(l.url || '');
      const text = escapeHtml(l.text || l.title || '');
      return `<a href="${url}" class="p-3.5 rounded-2xl border border-brand-beige hover:border-brand-navy hover:bg-brand-navy-light/20 transition-all text-xs font-semibold text-gray-800 flex items-center justify-between">
        <span>${text}</span>
        <span class="text-brand-navy font-bold">&rarr;</span>
      </a>`;
    }).join('\n');
    internalLinksHtml = `<section class="bg-white border border-brand-navy/10 rounded-3xl p-6 mt-8">
      <h3 class="font-title font-bold text-brand-navy text-base mb-3 flex items-center gap-2">
        <span>🔗</span> İlgili İçerikler ve Bağlantılı Rehberler
      </h3>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">${links}</div>
    </section>`;
  }

  let checklistHtml = '';
  if (Array.isArray(guide.checklist) && guide.checklist.length > 0) {
    const listItems = guide.checklist.map(item => `<li class="flex items-start gap-2"><span>✔</span><span>${escapeHtml(item)}</span></li>`).join('\n');
    checklistHtml = `<section class="bg-brand-navy-light/30 border-2 border-brand-navy/15 rounded-3xl p-6 mt-8">
      <h3 class="font-title font-bold text-brand-navy text-base mb-4 flex items-center gap-2">
        <span>✓</span> Uygulama ve Kontrol Listesi
      </h3>
      <ul class="space-y-2 text-xs md:text-sm text-gray-700">${listItems}</ul>
    </section>`;
  }

  const authorAvatar = guide.author?.imageUrl
    ? `<img src="${escapeHtml(guide.author.imageUrl)}" alt="${escapeHtml(authorName)}" class="w-10 h-10 rounded-full object-cover" width="40" height="40" />`
    : `<div class="w-10 h-10 rounded-full bg-brand-navy text-white flex items-center justify-center font-bold text-sm">🐾</div>`;

  const vetHtml = guide.vetChecked
  ...
  const shortAnswerHtml = guide.shortAnswer
    ...
    <main class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 text-left">
      <nav aria-label="breadcrumb" class="mb-6 py-2.5 px-4 bg-brand-navy-light/40 border-b border-brand-beige rounded-2xl">
        <ol class="flex items-center flex-wrap gap-2 text-xs font-bold text-gray-600">
          <li><a href="/" class="hover:underline text-gray-600">Ana Sayfa</a></li>
          <li class="text-gray-400">/</li>
          <li><a href="/blog" class="hover:underline text-gray-600">Blog</a></li>
          ${clusterBreadcrumb}
          <li class="text-gray-400">/</li>
      ...
      <article class="space-y-6">
        <header class="space-y-3">
          <div class="flex flex-wrap items-center gap-2.5 text-xs font-semibold text-gray-500">
            ${clusterBadge}
            <span class="bg-brand-navy-light text-brand-navy px-3 py-1 rounded-full font-bold uppercase text-3xs tracking-wider">${escapeHtml(guide.category || 'Rehber')}</span>
            <span>Yayınlanma: ${escapeHtml(guide.publishedAt || '2026-09-18')}</span>
            <span>&bull;</span>
            <span>Güncelleme: ${escapeHtml(guide.updatedAt || guide.publishedAt || '2026-09-18')}</span>
          </div>
          ...
          <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 py-4 border-y border-brand-beige mt-4">
            <div class="flex items-center gap-3">
              ${authorAvatar}
              <div>
                <p class="text-sm font-bold text-gray-800">${escapeHtml(authorName)}</p>
                <p class="text-xs text-gray-500">${escapeHtml(authorRole)}</p>
              </div>
            </div>
            ${vetHtml}
          </div>
...
export function renderCategoryOrClusterPreRenderHtml({
  ...
  const crossLinksHtml = `
    <section class="space-y-6 text-left border-t border-brand-beige pt-8">
      ...
      <div class="space-y-3">
        <h3 class="text-xs font-bold text-gray-500 uppercase tracking-wider">📚 Amiral Rehberler & Kılavuzlar</h3>
        <div class="flex flex-wrap gap-2 text-xs">
          <a href="/blog/evcil-hayvan-kabul-eden-oteller" class="px-3.5 py-1.5 bg-brand-cream border border-brand-yellow text-brand-navy font-bold rounded-lg hover:bg-brand-yellow">📖 Evcil Hayvan Kabul Eden Oteller: Türkiye Tatil Rehberi</a>
          <a href="/blog/kedi-kopek-kabul-eden-oteller" class="px-3.5 py-1.5 bg-brand-cream border border-brand-yellow text-brand-navy font-bold rounded-lg hover:bg-brand-yellow">🐾 Kedi ve Köpek Kabul Eden Oteller Rehberi</a>
          <a href="/evcil-hayvan-seyahat-rehberi" class="px-3.5 py-1.5 bg-white border border-brand-beige text-brand-navy font-bold rounded-lg hover:border-brand-navy">✈️ Tüm Seyahat Rehberleri</a>
        </div>
      </div>
      ...
      <div class="space-y-3">
        ...
        <div class="flex flex-wrap gap-2 text-xs">
          <a href="/antalya/evcil-hayvan-dostu-oteller" class="px-3 py-1.5 bg-white border border-brand-beige rounded-lg hover:border-brand-navy text-gray-700 font-medium">Antalya / Kaş / Kemer</a>
          <a href="/mugla/evcil-hayvan-dostu-oteller" class="px-3 py-1.5 bg-white border border-brand-beige rounded-lg hover:border-brand-navy text-gray-700 font-medium">Muğla / Bodrum / Fethiye</a>
          <a href="/izmir/evcil-hayvan-dostu-oteller" class="px-3 py-1.5 bg-white border border-brand-beige rounded-lg hover:border-brand-navy text-gray-700 font-medium">İzmir / Çeşme / Alaçatı</a>
          <a href="/sapanca/evcil-hayvan-dostu-bungalovlar" class="px-3 py-1.5 bg-white border border-brand-beige rounded-lg hover:border-brand-navy text-gray-700 font-medium">Sapanca Göl Bungalovları</a>
          <a href="/aydin/evcil-hayvan-dostu-oteller" class="px-3 py-1.5 bg-white border border-brand-beige rounded-lg hover:border-brand-navy text-gray-700 font-medium">Aydın / Kuşadası / Didim</a>
          <a href="/balikesir/evcil-hayvan-dostu-oteller" class="px-3 py-1.5 bg-white border border-brand-beige rounded-lg hover:border-brand-navy text-gray-700 font-medium">Balıkesir / Cunda / Ayvalık</a>
          <a href="/canakkale/evcil-hayvan-dostu-oteller" class="px-3 py-1.5 bg-white border border-brand-beige rounded-lg hover:border-brand-navy text-gray-700 font-medium">Çanakkale / Bozcaada / Assos</a>
          <a href="/istanbul/evcil-hayvan-dostu-oteller" class="px-3 py-1.5 bg-white border border-brand-beige rounded-lg hover:border-brand-navy text-gray-700 font-medium">İstanbul / Şile / Ağva</a>
          <a href="/bolu/evcil-hayvan-dostu-oteller" class="px-3 py-1.5 bg-white border border-brand-beige rounded-lg hover:border-brand-navy text-gray-700 font-medium">Bolu / Abant / Yedigöller</a>
          <a href="/kapadokya/evcil-hayvan-dostu-oteller" class="px-3 py-1.5 bg-white border border-brand-beige rounded-lg hover:border-brand-navy text-gray-700 font-medium">Kapadokya Mağara Otelleri</a>
        </div>
      </div>
  ...
  return `
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
      ...
      <header class="text-left space-y-3">
        ...
        <div class="mt-4 p-4 bg-amber-50/90 border border-amber-200/80 rounded-2xl text-xs text-amber-900 flex items-start gap-2.5 text-left">
          <span class="text-base leading-none">⚠️</span>
          <div>
            <strong class="font-bold">Önemli Güven Uyarısı:</strong> Evcil hayvan kabul koşulları ve ücretler değişebilir. Rezervasyon yapmadan önce evcil hayvanınızın türünü, kilosunu ve sayısını belirterek güncel kuralları doğrudan işletmeden doğrulayın.
          </div>
        </div>
      </header>
...
export function renderServicePreRenderHtml({ serviceType, items = [] }) {
      ...
      <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
        <nav aria-label="breadcrumb" class="mb-6 py-2.5 px-4 bg-brand-navy-light/40 border-b border-brand-beige rounded-2xl">
          <ol class="flex items-center flex-wrap gap-2 text-xs font-bold text-gray-600">
            <li><a href="/" class="hover:underline text-gray-600">Ana Sayfa</a></li>
            <li class="text-gray-400">/</li>
            <li class="text-brand-navy font-extrabold" aria-current="page">Kedi ve Köpek Otelleri</li>
          </ol>
        </nav>

        <header class="text-left space-y-3">
          <div class="inline-flex items-center gap-2 px-3 py-1 bg-brand-cream rounded-full text-xs font-bold text-brand-c2">
            <span>🛡️ Doğrudan Doğrulanmış Pet Otelleri Rehberi</span>
          </div>
          <h1 class="text-2xl sm:text-4xl font-bold font-title text-brand-navy">Türkiye Pet Otelleri & Kedi-Köpek Pansiyonları</h1>
          <p class="text-sm sm:text-base text-gray-600 max-w-4xl leading-relaxed">
            Türkiye genelindeki 131 doğrulanmış kedi ve köpek otelini tüm fotoğrafları, telefonları, adresleri ve Google misafir puanlarıyla karşılaştırın. 7/24 kamera, kafessiz odalar ve veteriner desteği.
          </p>
        </header>
        ...
        <section class="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
          ${items.slice(0, 9).map(item => `
            <article class="bg-white border border-brand-beige rounded-2xl p-5 space-y-3 shadow-xs hover:shadow-md transition-shadow">
              <span class="text-3xs bg-brand-navy-light px-2.5 py-1 rounded-full font-bold text-brand-navy">${escapeHtml(item.category || item.serviceType || 'Pet Oteli')}</span>
              <h3 class="font-title text-base font-bold text-brand-navy">
                <a href="${getBoardingPath(item)}" class="hover:underline">${escapeHtml(item.name)}</a>
              </h3>
              <p class="text-xs text-gray-500">📍 ${escapeHtml(item.district || '')}, ${escapeHtml(item.city || '')}</p>
              <p class="text-xs text-gray-600 line-clamp-2 leading-relaxed">${escapeHtml(item.description || 'Doğrulanmış evcil hayvan konaklama tesisi.')}</p>
              <div class="pt-2 text-3xs text-emerald-700 font-bold">✓ Google Maps Doğrulamalı</div>
            </article>
          `).join('')}
        </section>
      </main>
  ...
  if (serviceType === 'kopek-parklari') {
    const guides = DOG_WALKING_GUIDES;
    return `
      <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-10">
        <nav aria-label="breadcrumb" class="mb-6 py-2.5 px-4 bg-brand-navy-light/40 border-b border-brand-beige rounded-2xl">
          <ol class="flex items-center flex-wrap gap-2 text-xs font-bold text-gray-600">
            <li><a href="/" class="hover:underline text-gray-600">Ana Sayfa</a></li>
            <li class="text-gray-400">/</li>
            <li class="text-brand-navy font-extrabold" aria-current="page">Köpek Parkları ve Yürüyüş Rotaları</li>
          </ol>
        </nav>

        <header class="text-left space-y-3">
          <div class="inline-flex items-center gap-2 px-3 py-1 bg-brand-cream rounded-full text-xs font-bold text-brand-c2">
            <span>🐾 patili.co Açık Alan & Park Rehberi</span>
          </div>
          <h1 class="text-2xl sm:text-4xl font-bold font-title text-brand-navy">İstanbul, Ankara, İzmir & Antalya Köpek Parkları ve Yürüyüş Rotaları</h1>
          <p class="text-sm sm:text-base text-gray-600 max-w-4xl leading-relaxed">
            İstanbul, Ankara, İzmir ve Antalya'da doğrulanmış 32 köpek parkı, evcil hayvan parkuru ve tasmalı yürüyüş rotası. İlçe bazlı adresler, Google Haritalar yol tarifleri ve saha kuralları.
          </p>
        </header>

        <!-- Passage-First Direct Answer Block -->
        <section class="border-2 border-brand-navy/15 rounded-2xl p-6 bg-brand-navy-light/25 space-y-3 text-left">
          <h2 class="text-lg font-bold font-title text-brand-navy">Türkiye'de Köpek Gezdirme ve Park Kuralları Rehberi</h2>
          <div class="bg-white p-4 rounded-xl border border-brand-beige">
            <p class="text-xs sm:text-sm text-gray-800 leading-relaxed font-medium">
              <strong>Özet & Doğrudan Bilgi:</strong> patili.co Köpek Gezdirme ve Park Rehberi; İstanbul (10), Ankara (8), İzmir (8) ve Antalya (6) olmak üzere 4 büyük şehirde toplam 32 doğrulanmış konumu listeler. Alanlar 3 türe ayrılır: Özel köpek alanları (çevrili ve tasmasız serbest oyun imkanı sunan bölümler), evcil hayvan parkurları (belediye parkları içindeki çeviklik ekipmanları) ve tasmalı yürüyüş rotaları (sahil bandı, vadi ve korular). Her alanın tam adresi, Google Haritalar konumu ve editör doğrulama notları yer almaktadır.
            </p>
          </div>
        </section>

        <!-- Preset Chips -->
        <section class="flex flex-wrap gap-2 text-xs font-medium">
          <span class="px-3 py-1.5 bg-brand-navy text-white rounded-xl font-bold">Tüm Şehirler (32)</span>
          <span class="px-3 py-1.5 bg-brand-cream border border-brand-beige rounded-xl text-brand-navy">İstanbul (10)</span>
          <span class="px-3 py-1.5 bg-brand-cream border border-brand-beige rounded-xl text-brand-navy">Ankara (8)</span>
          <span class="px-3 py-1.5 bg-brand-cream border border-brand-beige rounded-xl text-brand-navy">İzmir (8)</span>
          <span class="px-3 py-1.5 bg-brand-cream border border-brand-beige rounded-xl text-brand-navy">Antalya (6)</span>
        </section>

        <!-- 32 Curated Dog Walking Locations -->
        <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-left">
          ${guides.map(guide => {
            const venue = VENUE_TYPES[guide.venueType] || {};
            return `
              <article class="bg-white border border-brand-beige rounded-2xl p-5 space-y-3 shadow-xs hover:shadow-md transition-shadow flex flex-col justify-between">
                <div class="space-y-3">
                  <div class="flex items-center justify-between gap-2">
                    <span class="text-3xs px-2.5 py-1 rounded-full font-bold border ${venue.badgeClass || 'bg-gray-100 text-gray-800'}">
                      ${venue.icon || '🐾'} ${escapeHtml(venue.label || guide.venueType)}
                    </span>
                    <span class="text-3xs font-bold text-gray-500">📍 ${escapeHtml(guide.district)}, ${escapeHtml(guide.city)}</span>
                  </div>
                  <h3 class="font-title text-base font-bold text-brand-navy">
                    <a href="${escapeHtml(guide.googleMapsUrl)}" target="_blank" rel="noopener noreferrer" class="hover:underline">
                      ${escapeHtml(guide.name)}
                    </a>
                  </h3>
                  <p class="text-xs text-gray-500 font-mono text-3xs">📌 ${escapeHtml(guide.address)}</p>
                  <p class="text-xs text-gray-700 leading-relaxed">${escapeHtml(guide.bodyText)}</p>
                  ${guide.verificationNote ? `
                    <div class="p-2.5 bg-amber-50/80 border border-amber-200/80 rounded-xl text-3xs text-amber-900 leading-relaxed">
                      <strong>⚠️ Saha Uyarısı:</strong> ${escapeHtml(guide.verificationNote)}
                    </div>
                  ` : ''}
                </div>
                <div class="pt-3 border-t border-brand-beige flex items-center justify-between text-xs">
                  <span class="text-3xs text-gray-400">Kaynak: ${escapeHtml(guide.source)}</span>
                  <a href="${escapeHtml(guide.googleMapsUrl)}" target="_blank" rel="noopener noreferrer" class="text-3xs font-bold text-brand-c2 hover:underline">
                    Haritada Aç &rarr;
                  </a>
                </div>
              </article>
        ...
        <!-- Editorial Checklist -->
        <section class="p-6 bg-brand-cream/30 border border-brand-beige rounded-2xl text-left space-y-4">
          <h2 class="text-lg font-bold font-title text-brand-navy">11 Maddelik Saha ve Yayın Doğrulama Kontrol Listesi</h2>
          <p class="text-xs text-gray-700 leading-relaxed">
            patili.co içeriğinde yer alan her bir park ve rota; çift kapı kilit sistemi, çit yüksekliği, zemin güvenliği, sokak hayvanı yoğunluğu, taze su kaynağı ve belediye zabıta tabelaları gibi 11 kritik güvenlik standardı üzerinden doğrulanır. Tasmasız serbest alanlar ancak tam çevrili ve güncel tabelası olan bölgelerde geçerlidir.
          </p>
        </section>
      </main>
    `;
