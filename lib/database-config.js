...
export function databaseConfig(connectionString, env = process.env) {
  ...
  const serverless = env.VERCEL === '1' || Boolean(env.VERCEL_ENV);
  if (serverless && isSupabasePoolerHost(parsed.hostname) && (parsed.port === '' || parsed.port === '5432') && env.DATABASE_POOL_MODE !== 'session') {
    parsed.port = '6543';
  }
