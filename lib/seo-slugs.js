...
export function getVetPath(vet) {
  if (!vet) return '/veterinerler';

  const city = slugify(vet.city) || 'turkiye';
  const district = slugify(vet.district) || 'merkez';
  const name = slugify(vet.name) || slugify(vet.id) || 'veteriner';

  return `/veteriner/${city}/${district}/${name}`;
}

export function getBoardingPath(boarding) {
  if (!boarding) return '/kedi-kopek-otelleri';

  const city = slugify(boarding.city) || 'turkiye';
  const district = slugify(boarding.district) || 'merkez';
  const name = slugify(boarding.name) || slugify(boarding.id) || 'pet-oteli';

  return `/kedi-kopek-oteli/${city}/${district}/${name}`;
}

export function findBoardingBySlugs(boardings = [], citySlug, districtSlug, boardingSlug) {
  if (!boardings || !boardings.length) return null;
  const targetBoardingSlug = slugify(boardingSlug || districtSlug || citySlug);
  const targetCitySlug = slugify(citySlug);
  const targetDistrictSlug = slugify(districtSlug);

  // 1. Exact match on city + district + name slug
  let match = boardings.find(b => (
    slugify(b.city) === targetCitySlug
    && slugify(b.district) === targetDistrictSlug
    && slugify(b.name) === targetBoardingSlug
  ));

  if (match) return match;

  // 2. Fallback: match by name slug or ID
  match = boardings.find(b => (
    slugify(b.name) === targetBoardingSlug
    || b.id === targetBoardingSlug
    || b.id === boardingSlug
  ));
...
export function findHotelBySlugs(hotels = [], citySlug, districtSlug, hotelSlug) {
  ...
  const targetHotelSlug = slugify(hotelSlug || districtSlug || citySlug);
  const targetCitySlug = slugify(citySlug);
  const targetDistrictSlug = slugify(districtSlug);

  // 1. Exact match on city + district + name slug
  let match = hotels.find(hotel => (
    slugify(hotel.city) === targetCitySlug
    && slugify(hotel.district) === targetDistrictSlug
    && slugify(hotel.name) === targetHotelSlug
  ));

  if (match) return match;

  // 2. Fallback: match by hotel name slug or ID
  match = hotels.find(hotel => (
    slugify(hotel.name) === targetHotelSlug
    || hotel.id === targetHotelSlug
    || hotel.id === hotelSlug
  ));
...
export const PROGRAMMATIC_CLUSTERS = [
  {
    ...
    metaTitle: 'Her Şey Dahil Evcil Hayvan Kabul Eden Oteller (2026 Tesisler) | patili.co',
  ...
  },
  {
    ...
    metaTitle: 'Büyük Köpek Kabul Eden Oteller & Kilo Sınırsız Tesisler | patili.co',
  ...
  },
  {
    ...
    metaTitle: 'Ek Ücret Almayan Evcil Hayvan Dostu Oteller (Ücretsiz Pet) | patili.co',
  ...
  },
  {
    ...
    metaTitle: 'Bahçeli Evcil Hayvan Dostu Oteller & Geniş Çim Alanlı Tesisler | patili.co',
  ...
  },
  {
    ...
    metaTitle: 'Evcil Hayvan Dostu Bungalovlar & Köpek Kabul Eden Tesisler | patili.co',
  ...
  },
  {
    ...
    metaTitle: 'Köpek Kabul Eden Bungalovlar & Doğa Evleri | patili.co',
  ...
  },
  {
    ...
    metaTitle: 'Evcil Hayvan Dostu Villalar & Köpek Kabul Eden Kiralık Tesisler | patili.co',
  ...
  },
  {
    ...
    metaTitle: 'Evcil Hayvan Dostu Butik Oteller (Seçkin Tesisler) | patili.co',
  ...
  },
  {
    ...
    metaTitle: 'Evcil Hayvan Dostu Tatil Köyleri & Resort Oteller | patili.co',
    metaDesc: 'Geniş arazide evcil hayvan kabul eden tam donanımlı tatil köyleri ve lüks resort oteller.',
    accType: 'Tatil Köyü'
  },
  {
    id: 'kedi',
    slug: 'kedi-kabul-eden-oteller',
    title: 'Kedi Kabul Eden Oteller',
    h1: 'Kedi Kabul Eden Evcil Hayvan Dostu Oteller',
    metaTitle: 'Kedi Kabul Eden Oteller & Kedi Dostu Tesisler | patili.co',
    metaDesc: 'Kedinizle rahatça konaklayabileceğiniz kedi kabul eden otel, pansiyon ve tatil evleri. Aşı karnesi ve oda kuralları.',
    petType: 'cat'
  },
  {
    id: 'kopek',
    slug: 'kopek-kabul-eden-oteller',
    title: 'Köpek Kabul Eden Oteller',
    h1: 'Köpek Kabul Eden Evcil Hayvan Dostu Oteller',
    metaTitle: 'Köpek Kabul Eden Oteller & Köpek Dostu Tesisler | patili.co',
    metaDesc: 'Köpeğinizle konaklayabileceğiniz en iyi köpek dostu oteller, butik oteller ve tatil köyleri. Irk ve kilo politikaları.',
    petType: 'dog'
  }
];
